      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-16">
          ...
          <div className="relative">
            {/* Timeline line */}
            <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-pink-500/50 via-yellow-400/50 to-green-500/50 hidden md:block"></div>
            ...
            {instructions.map((instruction, index) => (
              <div key={index} className={`flex items-center ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} flex-col md:space-x-8`}>
                ...
                <div className={`flex-1 ${index % 2 === 0 ? 'md:text-right md:pr-8' : 'md:text-left md:pl-8'} text-center md:text-inherit`}>
                  <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 hover:border-pink-500/30 hover:shadow-xl hover:shadow-pink-500/10 transition-all duration-300 transform hover:-translate-y-1">
                    <div className="space-y-4">
                      <div className="flex items-center justify-center md:justify-start space-x-3">
                        ...
                        <h4 className="text-2xl font-bold text-gray-900">{instruction.title}</h4>
                      </div>
                      <p className="text-lg text-gray-600 font-medium">{instruction.description}</p>
                      <p className="text-sm text-gray-500 leading-relaxed">{instruction.detail}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="relative z-10 flex-shrink-0 my-8 md:my-0">
                  <div className={`w-16 h-16 bg-gradient-to-br ${instruction.gradient} rounded-full flex items-center justify-center shadow-lg border-4 border-white`}>
                    <span className="text-white font-bold text-xl">{instruction.step}</span>
                  </div>
                  {/* Timeline connector dots */}
                  {index < instructions.length - 1 && (
                    <div className="absolute top-16 left-1/2 transform -translate-x-1/2 w-2 h-8 bg-gradient-to-b from-gray-300 to-transparent rounded-full md:hidden"></div>
                  )}
                </div>
